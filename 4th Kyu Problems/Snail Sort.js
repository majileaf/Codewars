...
highest; the idea is to traverse the 2-d array in a clockwise snailshell pattern.
...
const snail = array => {
    ...
    let arr = [];
    let i = j = off = idirection = 0;
    let jdirection = 1;

    const n = array.length - 1;
    let nxn = array.length ** 2;

    while (nxn--) {
        arr.push(array[i][j]);

        if (j === n - off && jdirection === 1) {
            jdirection = 0;
            idirection = 1;
        } else if (i === n - off && idirection === 1) {
            idirection = 0;
            jdirection = -1;
        } else if (j === 0 + off && jdirection === -1) {
            jdirection = 0;
            idirection = -1;
            off++;
        } else if (i === 0 + off && idirection === -1) {
            idirection = 0;
            jdirection = 1;
        }

        (idirection === 0) ? i : (idirection === 1) ? i++ : i--;
        (jdirection === 0) ? j : (jdirection === 1) ? j++ : j--;
    }
    return arr;
}
