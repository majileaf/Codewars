...
He started with summing and did it easily, but decided to store the sum he found in some random position of the original array 
which was a bad idea. Now he needs to cope with the second task, sorting the original array arr, and it's giving him trouble 
since he modified it.

Given the array shuffled, consisting of elements a1, a2, ..., an, and their sumvalue in random order, return the sorted array 
of original elements a1, a2, ..., an.
...
const shuffledArray = shuffled => {
    const total = shuffled.reduce((sum, n) => sum + n, 0);
    const sumIdx = shuffled.findIndex(e => 2 * e === total);
    const res = shuffled.slice();
    res.splice(sumIdx, 1);
