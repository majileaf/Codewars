...
One number might have deleted from it, then the remaining 
...
// initial solution:
// const findDeletedNumber = (arr, mixArr) => arr.find(e => !mixArr.includes(e)) || 0;

const findDeletedNumber = (arr, mixArr) => (arr.length === mixArr.length) 
    ? 0
    : arr.length * (arr.length + 1) / 2 - mixArr.reduce((sum, n) => sum + n, 0);
