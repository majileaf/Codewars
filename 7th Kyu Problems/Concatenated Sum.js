/* Concatenated Sum
The number 198 has the property that 198 = 11 + 99 + 88, i.e., if each of its digits is concatenated twice and 
then summed, the result will be the original number. It turns out that 198 is the only number with this property. 
...
const checkConcatenatedSum = (original, times) => Math.abs(original) === [...String(Math.abs(original))].reduce((sum, n) => sum + Number(n.repeat(times)), 0);
