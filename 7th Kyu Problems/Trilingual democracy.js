...
const trilingualDemocracy = group => {
  const count = [...group].reduce((sum, n) => (sum[n] = sum[n] + 1, sum), {D: 0, F: 0, I: 0, K: 0});
  const max = Math.max(...Object.values(count));
  return max === 3 ? group[0] :
         max === 2 ? Object.keys(count).find(e => count[e] === 1) :
         Object.keys(count).find(e => count[e] === 0);
}
