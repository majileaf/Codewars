...
The mean should be computed if both x and y have values higher than 1 but less or 
equal to the array's length. Otherwise the function should return -1.

Examples
[1, 3, 2, 4], 2, 3 => should return 2.5
because: the mean of the the first 2 elements of the array is (1+3)/2=2 and 
...
const sumMean = arr => arr.reduce((sum, n) => sum + n, 0) / arr.length;
const getMean = (arr, x, y) => {
    if (x <= 1 || y <= 1 || x > arr.length || y > arr.length) return -1;
    return (sumMean(arr.slice(0, x)) + sumMean(arr.slice(-y))) / 2;
}
