...
abs() can has only one parameter, max() and min() can has more than one parameters. 
...
We can see: For max() and min(). If it has only one parameter, method returns the parameter itself; 
If it has many parameters, method will return the maximum / minimum values; 
For abs(). No matter how many parameters, it returns only the absolute value of the first parameter.

max() and min() can accept an array as a parameter by using spread operator (...). 

The following example is a classic usage, to calculate the maximum/minimum values of the array:
var arr=[1,2,3,4,5];
