/* Pandemia
⚠️ The world is in quarantine! There is a new pandemia that struggles mankind. Each continent is isolated from each other 
but infected people have spread before the warning. ⚠️
...
const infected = s => {
    const infectedPeople = s.split('X')
                            ...
                            .split('')
                            .filter(e => e === '1').length;
    const totalInfected = s.split('X').join('').length;
    return infectedPeople / totalInfected * 100 || 0;
}
