...
For each one of these 3 categories they are going to be awarded one point, should they get a better judgement from 
the jury. No point is awarded in case of an equal vote.
...
Example:
Alice's band plays a Nirvana inspired grunge and has been rated 20 for Heaviness, 32 for Originality and 
only 18 for Outfits. Bob listens to Slayer and has gotten a good 48 for Heaviness, 25 for Originality and 
a rather honest 40 for Outfits.
...
const solve = (a, b) => {
    const [alice, bob] = a.reduce(([scoreA, scoreB], _, i) => (a[i] > b[i] ? scoreA++ : a[i] < b[i] ? scoreB++ : null, [scoreA, scoreB]), [0, 0]);
