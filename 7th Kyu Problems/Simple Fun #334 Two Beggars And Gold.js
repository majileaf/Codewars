...
In the field, two beggars A and B found some gold at the same time. 
They all wanted the gold, and they decided to use simple rules to distribute gold:
...
Given an integer array golds, and assume that A always takes first. 
Please calculate the final amount of gold obtained by A and B. 
returns a two-element array [amount of A, amount of B].
...
const distributionOf = golds => {
    ...
    while (golds.length) {
        golds[0] >= golds.at(-1) ? left  += golds.shift()      : left  += golds.pop();
        golds[0] >= golds.at(-1) ? right += golds.shift() || 0 : right += golds.pop() || 0;
    }
    return [left, right];
}
...
console.log(distributionOf([12,11,11,28,7])) // [30, 39]