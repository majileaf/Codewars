...
const emoMap = {':)': 0, ':D': 1, '>(': 2, '>:C':3, ':/': 4, ':|': 5, ':O': 6, ';)': 7, '^.^':8, ':(': 9};
const deEmojify = emojiString => emojiString.length ? String.fromCharCode(...emojiString.split('  ').map(e => e.split(' ').map(f => emoMap[f]).join(''))) : '';
