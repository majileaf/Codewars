...
To make it a bit more clear: e.g. in case of "abcdefgh" as the second optional parameter, you replace "a" with "h", 
"b" with "g" etc. .
...
const mirror = (code, opt = 'abcdefghijklmnopqrstuvwxyz') => {
    const optMap = [...opt].reduce((sum, n, i) => (sum[n] = opt[opt.length - 1 - i], sum), {});
    return code.toLowerCase().split('').map(letter => optMap[letter] || letter).join('');
}
