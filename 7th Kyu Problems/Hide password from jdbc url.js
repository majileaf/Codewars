...
We have to create a function that receives a connection string with password included and you have to mask the password 
i.e. change password by asterisks.
...
const hidePasswordFromConnection = urlString => urlString.replace(/(?<=&password=).*|(?<=\?password=).*(?=&)/, char => '*'.repeat(char.length));
