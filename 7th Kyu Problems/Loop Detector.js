/* Loop Detector
In this kata, you'll simulate traversing a one-way pointer chain, similar to a singly linked list. Each element in the 
input list represents the index of the next element to move to.
...
const hasLoop = indices => {
    const loopMap = {0: 1};
    let i = 0;
    while (!loopMap[indices[i]]) {
        if (indices[i] >= indices.length || indices[i] === undefined) return false;
        loopMap[indices[i]] = 1;
        i = indices[i];
    }
    return true;
...
console.log(hasLoop([1,2,3,4,2])) // true
console.log(hasLoop([1,2,3,4,5])) // false
console.log(hasLoop([0])) // true
console.log(hasLoop([3,2,1,6])) // false
console.log(hasLoop([1,0])) // true
console.log(hasLoop([3,2,1,4])) // false
console.log(hasLoop([2,0,1,5])) // true
console.log(hasLoop([])) // false