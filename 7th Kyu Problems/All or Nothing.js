...
Given a potentially incomplete answer key and the student's answers, write a function that 
determines whether or not a student can still score 100%. Incomplete questions are marked with an underscore, "_".
...
const possiblyPerfect = (key, answers) => {
    const allTrue  = key.every((_, i) => key[i] === answers[i] || key[i] === '_');
    const allFalse = key.every((_, i) => key[i] !== answers[i] || key[i] === '_');
