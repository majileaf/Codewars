/* Time Degrees
Time , time , time . Your task is to write a function that will return the degrees on a analog clock from 
a digital time that is passed in as parameter . The digital time is type string and will be in the format 00:00 . 
...
//   [hour, min] = [+hour, +min];
//   if (!Number.isInteger(hour) || !Number.isInteger(min) || hour < 0 || hour > 23 || min < 0 || min > 59) return 'Check your time !';
//   return `${(hour % 12 || 12) * 30}:${(min % 60 || 60) * 6}`;
// }
