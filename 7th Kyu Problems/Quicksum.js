...
are often used for detecting transmission errors, validating document contents, and 
...
the character’s value. A space has a value of zero, while letters have a value equal to 
their position in the alphabet.

So, A = 1, B = 2, etc., through Z = 26. Here are example Quicksum calculations for the 
packets "ACM" and "A C M":
...
A C M
1 x 1 + 3 x 3 + 5 * 13 = 75
When the packet doesn't have only uppercase letters and spaces or just spaces the result to 
quicksum have to be zero (0).
...
const quicksum = packet => {
  ...
  return [...packet].reduce((sum, n, i) => sum + (i + 1) * (n === ' ' ? 0 : n.charCodeAt() - 64), 0);
}
