...
Usage is arrayObject.reverse() and don't need parameters. 
...
The parameter compareFunction is optional. Specifies a function that defines the sort order. 
...
This is an example of an array of strings, if arr is a number array and the parameter is omitted, 
...
This result may not be what we want. If you want to sort the numbers according to the size of their values, 
you must use a parameter, the parameter is a function. 
...
The formal format is: function(a,b){return a-b}. If you forget how to use the arrow function, 
...
If we need to sort by two or more conditions, we need to spend more energy to write a proper comparison function. 
The following example has two sorting conditions, first according to the classification of odd and even order, 
...
const sortIt = arr => {
  ...
  return arr.slice().sort((a, b) => count[a] - count[b] || b - a);
}
