...
We implement here a very simple model of bus mastering. Given n, a number representing the number of masters connected to the bus, 
and a fixed priority order (the first master has more access priority than the second and so on...), 
the task is to choose the selected master. In practice, you are given a string inp of length n representing the n masters' requests 
...
const arbitrate = (input, n)  => {
    ...
    for (let i = 0, count = 0, occ = 1; i < input.length; i++) {
        if (input[i] === '1') count++;
        if (count !== occ && input[i] === '1') input[i] = '0';
    }
    return input.join('');
}

// alternatively:
// const arbitrate = (input, n) => input.replace(/(?<=1.*)1/g, '0');
