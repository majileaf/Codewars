...
in ohms and how tight a tolerance that value has. While you could always get a tattoo like Jimmie Rodgers to help 
you remember the resistor color codes, in the meantime, you can write a function that will take a string containing 
a resistor's band colors and return a string identifying the resistor's ohms and tolerance values.

The resistor color codes
You can see this Wikipedia page for a colorful chart, but the basic resistor color codes are:
black: 0, brown: 1, red: 2, orange: 3, yellow: 4, green: 5, blue: 6, violet: 7, gray: 8, white: 9

Each resistor will have at least three bands, with the first and second bands indicating the 
first two digits of the ohms value, and the third indicating the power of ten to multiply them by, 
for example a resistor with the three bands "yellow violet black" would be 47 * 10^0 ohms, or 47 ohms.

Most resistors will also have a fourth band that is either gold or silver, with gold indicating plus or 
minus 5% tolerance, and silver indicating 10% tolerance. Resistors that do not have a fourth band are 
rated at 20% tolerance. (There are also more specialized resistors which can have more bands and additional 
meanings for some of the colors, but this kata will not cover them.)
...
followed by a comma and a space, the tolerance value (5, 10, or 20), and a percent sign. 
...
const decodeResistorColors = bands => {
  const [r1, r2, p, t] = bands.split(' ');
  const resistorColorCode = {black: '0', brown: '1', red: '2', orange: '3', yellow: '4', green: '5', blue: '6', violet: '7', gray: '8', white: '9', silver: '10', gold: '5'};
  const totalOhm = (resistorColorCode[r1] + resistorColorCode[r2]) * 10 ** resistorColorCode[p];
  const newOhm = totalOhm >= 1e3 && totalOhm < 1e6 ? totalOhm / 1e3 + 'k' : totalOhm >= 1e6 ? totalOhm / 1e6 + 'M' : totalOhm;
  const tolerance = resistorColorCode[t] || 20;

  return `${newOhm} ohms, ${tolerance}%`;
}
...
console.log(decodeResistorColors("red red orange silver")) //           "22k ohms, 10%"
console.log(decodeResistorColors("yellow violet orange gold")) //       "47k ohms, 5%"
console.log(decodeResistorColors("brown black yellow gold")) //        "100k ohms, 5%"
console.log(decodeResistorColors("orange orange yellow gold")) //      "330k ohms, 5%"
