/* Move Zeros
Implement a function move_zeros which takes two arguments first an array of numbers arrNum and 
second a Boolean value isRight (default is true) and returns the array with all zero to right 
...
const move_zeros = (arrNum, isRight = true) => {
    const filtered = arrNum.filter(e => e !== 0);
    const zeros = Array(arrNum.length - filtered.length).fill(0);
    return isRight ? [...filtered, ...zeros] : [...zeros, ...filtered];
}
