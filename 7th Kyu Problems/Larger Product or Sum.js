...
You have to find the sum of the n largest numbers of the array and the 
product of the n smallest numbers of the array, and compare the two.
...
const sumOrProduct = (array, n) => {
  ...
  const sumHigh = numbers.slice(-n).reduce((sum, n) => sum + n, 0);
  const productLow = numbers.slice(0, n).reduce((pro, n) => pro * n, 1);

  return sumHigh > productLow ? 'sum' :
         sumHigh < productLow ? 'product' 
         : 'same';
}
