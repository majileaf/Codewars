...
If time is greater than space, then the moment is from the future. If time is less than space, 
then the moment is from the past. Otherwise, it is the present moment.
...
//     const [sumTime, sumSpace] = [...moment].reduce(([time, space], n) => (/[1-9]/g.test(n) ? [time += +n, space] : [time, space++], [time, space]), [0, 0]);
//     sumTime > sumSpace ? future = !future : sumTime < sumSpace ? past = !past : present = !present;
//     return [past, present, future];
// }

// alternatively:
const momentOfTimeInSpace = moment => {
  const [time, space] = [...moment].reduce(([sumTime, sumSpace], n) => (/[1-9]/g.test(n) ? [sumTime += +n, sumSpace] : [sumTime, sumSpace++], [sumTime, sumSpace]), [0, 0]);
