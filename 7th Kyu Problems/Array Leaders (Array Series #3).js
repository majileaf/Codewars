...
const arrayLeaders = numbers => numbers.reduce((sum, _, i, arr) => {
    if (arr[i] > arr.slice(i + 1).reduce((acc, pre) => acc + pre, 0)) sum.push(arr[i]);
    return sum;
}, []);

// alternatively:
// const arrayLeaders = numbers => numbers.filter((e, i) => e > numbers.slice(i + 1).reduce((sum, n) => sum + n, 0));
