...
index is even, therefore that character should be upper cased and 
you need to start over for each word.
...
const toWeirdCase = string => string
    .split(' ')
    .map(word => [...word].map((letter, i) => i % 2 === 0 ? letter.toUpperCase() : letter.toLowerCase()).join(''))
    .join(' ');
