...
Trolls: 5
Wizards: 10
Although weather, location, supplies and valor play a part in any battle, if you add up 
the worth of the side of good and compare it with the worth of the side of evil, the side 
with the larger worth will tend to win.
...
All values are non-negative integers. The resulting sum of the worth for each side will not 
...
const goodVsEvil = (good, evil) => {
    ...
    const goodResult = good.split(' ').reduce((sum, n, i) => sum + n * goodWorth[i], 0);
    const evilResult = evil.split(' ').reduce((sum, n, i) => sum + n * evilWorth[i], 0);

    return goodResult === evilResult ? 'Battle Result: No victor on this battle field' : 
           goodResult > evilResult   ? 'Battle Result: Good triumphs over Evil' : 
