...
As you can tell, each Student has some fives, tens, and twenties. Your job is to return the name of the student with
the most money. If every student has the same amount, then return "all".
...
const sumMoney = student => student.fives * 5 + student.tens * 10 + student.twenties * 20;
const mostMoney = students => {
    students.forEach(e => e['total'] = sumMoney(e));
    const maxMoney = Math.max(...students.map(e => e.total));
    return students.length > 1 && students.every(e => e.total === maxMoney) 
        ? 'all' 
        : students.find(e => e.total === maxMoney).name;
}
