...
To compare the results of the teams you are asked for giving three statistics; range, average and median.
    ...
    Median : In statistics, the median is the number separating the higher half of a data sample from the lower half. 
        ...
        If there is an even number of observations, then there is no single middle value; the median is then defined to be the mean 
...
const decodeStat = str => str.split('|').reduce((sum, n, i) => sum + n * 60 ** (2 - i), 0);
const encodeStat = n => [String(Math.floor(n / (60 * 60))).padStart(2, '0'), 
                         String(Math.floor(n % (60 * 60) / 60)).padStart(2, '0'), 
                         String(Math.floor(n % 60)).padStart(2, '0')].join('|');
const avgStat = arr => arr.reduce((sum, n) => sum + n, 0) / arr.length;
const stat = strg => {
    if (strg === '') return '';
    const arr = strg.split(', ').map(decodeStat).sort((a, b) => a - b);
    const range = arr[arr.length - 1] - arr[0];
    const avg = avgStat(arr);
    const len = arr.length / 2;
    const median = avgStat(arr.length % 2 ? arr.slice(len, len + 1) : arr.slice(len - 1, len + 1));
    return ['Range:', encodeStat(range), 'Average:', encodeStat(avg), 'Median:', encodeStat(median)].join(' ');
}
