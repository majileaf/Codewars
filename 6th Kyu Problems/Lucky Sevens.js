...
The surrounding numbers will be described as the numbers directly above, below, 
and next to (not diagonally) the number 7. You will be given a 2D array containing at least 1 lucky seven. 
...
const luckySevens = arr => {
    ...
    for (let i = 0; i < arr.length; i++) {
        for (let j = 0; j < arr[i].length; j++) {
            const sum = (i - 1 >= 0            ? arr[i - 1][j] : 0) 
                      + (j - 1 >= 0            ? arr[i][j - 1] : 0) 
                      + (i + 1 < arr.length    ? arr[i + 1][j] : 0) 
                      + (j + 1 < arr[i].length ? arr[i][j + 1] : 0);
            const isCube = Math.cbrt(sum) % 1 === 0;
            if (arr[i][j] === 7 && isCube) total++;
        }
    }
    return total;
}
