...
const getCode = str => {
    ...
    if (str.length === 1)      return str[0].slice(0, 4);
    else if (str.length === 2) return str.map(e => e.slice(0, 2)).join('');
    else if (str.length === 3) return (str[0][0] + str[1][0] + str[2].slice(0, 2));
    else if (str.length === 4) return str.map(e => e[0]).join('');
}

const birdCode = arr => arr.map(e => getCode(e));
