...
JavaScript's indexOf do es not work with arrays as input. This is because [1,2] === [1,2] will return false in JavaScript. 
Many other languages have similar quirks.

However, sometimes it is useful to search for an Array. Write a function that looks for an array within 
a two-dimensional array and returns the index of the first matching element. 
...
const searchArray = (arrayToSearch, query) => {
    if (!Array.isArray(arrayToSearch) || !arrayToSearch.every(e => e.length === 2) || query.length !== 2) throw new Error();
    const [qA, qB] = query;
    let i = 0;

    for (let [a, b] of arrayToSearch) {
        if (a === qA && b === qB) return i;
        i++
    }
    return -1;
}
...
console.log(searchArray(bigArray,[9,20])) // 2
console.log(searchArray(bigArray,[1,12])) // -1
console.log(searchArray(bigArray, [7,2])) // 1