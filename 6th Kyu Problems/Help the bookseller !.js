...
For the lists L and M of example you have to return the string (in Haskell/Clojure/Racket 
a list of pairs):

(A : 20) - (B : 114) - (C : 50) - (W : 0)
where A, B, C, W are the categories, 20 is the sum of the unique book of category A, 114 
the sum corresponding to "BKWRK" and "BTSQZ", 50 corresponding to "CDXEF" and 0 to 
...
const stockList = (listOfArt, listOfCat) => {
    ...
    const book = listOfArt.reduce((sum, n) => {
        const code = n.split(' ');
        [...code[0][0]].map(e => sum[e] = (sum[e] || 0) + Number(code[1]));
        return sum;
    }, {});

    return listOfCat.map(category => `(${category} : ${book[category] || 0})`).join(' - ');
}
