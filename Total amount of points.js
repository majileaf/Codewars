...
// Initial solution:
// const points = games => games.reduce((sum, n) => sum + (Number(n[0]) > Number(n[2]) ? 3 : Number(n[0]) === Number(n[2]) ? 1 : 0), 0);

const points = games => games.reduce((sum, n) => sum + (n[0] > n[2] ? 3 : n[0] === n[2] ? 1 : 0), 0);